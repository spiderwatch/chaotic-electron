...
async function generateWorkerClaimableCard() {
    ...
    if (workers.user) {
        ...
        tableHeadRow.innerHTML = "<th>Worker</th><th class=\"right\">Amount Ready to Claim</th>";
        tableHead.appendChild(tableHeadRow);
...
if (claimAllButton){
    claimAllButton.addEventListener("click", async function(e) {
        e.preventDefault();
        claimAllButton.innerHTML = "<i class=\"fa-sharp fa-ellipsis fa-flip fa-beat-fade\"></i>"
        claimAllButton.setAttribute("disabled", "");
        await fetch('/api/workers', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
